...
mediana de \textbf{8}, con una desviación típica de 3,28. El Anexo~C amplía
este análisis con el volumen por condición y medicamento, la longitud de las
reseñas y la evolución temporal.
...
Estándar (descarta negaciones)     & 0,9446 & 0,8900 & 0,9647 \\
\textbf{Conservando negaciones}    & \textbf{0,9493} & \textbf{0,8992} & \textbf{0,9698} \\ \bottomrule
\end{tabular}
\end{table}

La mejora más notable se produce en el \textbf{F1 macro} ($+0,9$ puntos), es
decir, justo en la detección de la clase minoritaria, que era la principal
debilidad del sistema. Con la lista estándar, \textit{«it did work»} e
\textit{«it did not work»} reciben ambas un 0,2\,\% de probabilidad de ser
positivas: son indistinguibles. Tras el cambio, la primera obtiene un
86,4\,\% y la segunda menos del 0,1\,\%.
...
macro (0,9008) refleja esta realidad mucho mejor que el F1 global. Las curvas
ROC y el desglose por clase de todos los modelos están en el Anexo~D.
...
\begin{enumerate}[leftmargin=*]
  \item \textbf{La representación importa más que el algoritmo.} Random Forest,
        ...
        conserva el 8,4\,\% de la varianza y destruye los términos poco
...
por defecto en la librería. Corregirla mejoró el F1 macro en 0,9 puntos. La
...
y puede verificarse volviendo a ejecutar el código. Esa ejecución se realizó
con Python 3.10, pandas 2.3.3, numpy 2.2.6 y scikit-learn 1.7.2. Con versiones
posteriores el código funciona sin cambios, pero al cargar el modelo ya
entrenado las probabilidades individuales pueden desplazarse unos puntos (la
reseña mixta de la sección~9.3 obtiene un 90,9\,\% con scikit-learn 1.7.2 y
un 96,2\,\% con 1.9.1, que es la versión con la que se tomaron las capturas).
Ninguna clasificación ni conclusión cambia, y las métricas globales se
reproducen reentrenando el modelo.
...
\texttt{temas\_y\_explicabilidad.json} & Temas NMF, aspectos y casos LIME \\
\texttt{ablacion\_negaciones.json}    & Efecto de las negaciones (sección~\ref{sec:negaciones}) y varianza SVD \\ \bottomrule
\end{tabular}
\end{center}
