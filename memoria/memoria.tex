...
de soporte lineal sobre TF-IDF— alcanza un \textbf{F1 de 0,9494} y un
\textbf{ROC-AUC de 0,9649}. Esa cifra no procede de una evaluación cómoda: el
...
\textbf{Qué no resuelve.} El sistema detecta el 96,1\,\% de las opiniones
favorables pero solo el 79,7\,\% de las desfavorables, que son justamente las
...
correctamente 6.922 opiniones desfavorables y explica por qué lo son. Revisar
...
(\texttt{joblib}, 6,6~MB) que encapsula el vectorizador y el clasificador. Esto
...
\begin{figure}[htbp]\centering
  ...
  \caption{Reseña desfavorable. El modelo asigna una probabilidad de sentimiento
           ...
           del 35\,\% y señala los términos decisivos: \textit{worst}
           ($-0{,}060$), \textit{severe} ($-0{,}049$) y \textit{unbearable}
           ($-0{,}041$).}
  \label{fig:app_neg}
\end{figure}
...
\begin{figure}[htbp]\centering
  ...
  \caption{Reseña mixta. El modelo la clasifica como positiva con un 96,2\,\%
           de confianza pese a que la paciente se plantea abandonar: el peso de
           \textit{works} (+0,181) domina sobre los términos de duda.}
  \label{fig:app_mixta}
\end{figure}
...
$+0{,}181$, casi el triple que el mayor de los términos que expresan duda
(\textit{pros}, $-0{,}070$; \textit{sure}, $-0{,}065$; \textit{continue},
$-0{,}060$). El sistema detecta el vocabulario de eficacia, pero no la reserva
que lo acompaña.
...
paciente —únicamente lo que escribe— el modelo alcanza un ROC-AUC de 0,9649. La
...
\begin{enumerate}[leftmargin=*]
  \item \textbf{La representación importa más que el algoritmo.} Random Forest,
        el único modelo capaz de captar relaciones no lineales, queda cuatro
...
\begin{itemize}[leftmargin=*]
  \item \textbf{El modelo detecta peor lo negativo}, que es lo que más importa:
        una de cada cinco reseñas desfavorables pasa desapercibida. Se mitiga
